...
%% options for saving audio files
...
savePath                = 'sampleSounds/';
vrs                     = '';   % string describing stimulus generation version, useful for de-bugging / playing with parameters
mkdir(savePath);

[design, stimuli] = fDesignStimuli(design); % this simply generates a vector of tone pip frequencies, NOT an audio file

%% generates audio from the sequences generated above & saves in the
% specified directory
if saveStim
    ...
    save(fullfile(savePath,['design' vrs '.mat']),'design'); % also saves a mat containing all the sequence info and the parameter settings
end